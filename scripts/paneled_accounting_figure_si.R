...
lake_mead = lc %>%
  ...
  mutate(year = year(date), 
         date = date(date)) %>%
  select(-SDI) %>%
  ggplot(aes(x = date, y = value*0.3048)) + # meter conversion
  geom_line() +
  scale_x_date(date_labels = "%Y", 
               date_breaks = "5 years", 
               limits = date(c("1964-01-01", "2022-08-01"))) +
  theme_classic(base_size = 10) +
  scale_y_continuous(limits = c(315, 385), name = "End of Year Elevation (m)", 
                     labels = scales::label_comma()) +
  geom_hline(yintercept = 1075*0.3048, lty = 2) +
  geom_hline(yintercept = 1090*0.3048, lty = 2) +
  ggplot2::annotate("text", x = date("2022-01-01"), y = 1110*0.3048, label = "Tier 0") +
  ggplot2::annotate("text", x = date("2022-01-01"), y = 1060*0.3048, label = 'Tier 1') +
...
excess_to_mx = mx_table_data %>%
  ...
  ggplot(aes(x = date, y = value*1233.48)) + # cubic meter conversion
  geom_line() +
  scale_y_continuous(
    name = bquote('Volume'~(m^3)),
    labels = scales::label_number(suffix = "B", scale = 1e-9, big.mark = ",")
  ) +
  scale_x_date(date_labels = "%Y", 
               date_breaks = "5 years", 
               limits = date(c("1964-01-01", "2022-08-01"))) +
...
az_excess = lc %>%
  ...
  ggplot(aes(x = date, y = diff*1233.48)) +
  geom_line() +
  scale_y_continuous(
    name = bquote('Volume'~(m^3)),
    labels = scales::label_number(suffix = "B", scale = 1e-9, big.mark = ",")
...
ggsave("./output/combined_overdelivery_plot_si.png", combined, width = 8.5, height = 11, units = "in")
